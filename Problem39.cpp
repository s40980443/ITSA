...
int main()
{
    ...
    int N;
    int hearing[100], read[100], speak[100];
    while (cin >> N)
    {
        for (int i = 0; i < N; i++)
        {
            cin >> hearing[i] >> read[i] >> speak[i];
        }

        for (int i = 0; i < N; i++)
        {
            if (hearing[i] >= 60 && read[i] >= 60 && speak[i] >= 60)
            {
                cout << "P" << endl;
            }
        }
        ...
        for (int i = 0; i < N; i++)
        {
            if (hearing[i] < 60 || read[i] < 60 || speak[i] < 60 && hearing[i] + read[i] + speak[i] > 220)
            {
                cout << "P" << endl;
            }
        }
